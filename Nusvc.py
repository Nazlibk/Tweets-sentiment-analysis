""" This file is the implementation of NuSVC. After fitting the regressor, this file uses pickle library
...
tfidf_vect = TfidfVectorizer(max_features=30000, ngram_range=(1, 1), stop_words="english", use_idf=True, smooth_idf=True, min_df=4)
X_train_tfidf =  tfidf_vect.fit_transform(X_train[:, 1])
X_test_tfidf =  tfidf_vect.fit_transform(X_test[:, 1])

# NuSVC
# I tuned the model with parameters below which are given best results. You can change the parameters if it is needed.
from sklearn.svm import NuSVC
regressor = NuSVC(degree=1, nu=0.5)
regressor.fit(X_train_tfidf, y_train)

# Serializing our model to a file called nuSVCModel.pkl
pickle.dump(regressor, open("nuSVCModel.pkl","wb"))
